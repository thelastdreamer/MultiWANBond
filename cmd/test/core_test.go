...
func main() {
	...
	fecEncoder := fec.NewEncoder(10, 2) // 10 data packets, 2 parity packets
	if fecEncoder != nil {
		fmt.Println("  ✓ FEC Encoder created (10 data + 2 parity)")

		// Create test data packets
		dataPackets := make([][]byte, 10)
		for i := 0; i < 10; i++ {
			dataPackets[i] = []byte(fmt.Sprintf("Test packet %d with some data", i))
		}

		// Encode
		parityPackets, err := fecEncoder.Encode(dataPackets)
		if err != nil {
			fmt.Printf("  ✗ FEC encoding failed: %v\n", err)
		} else {
			fmt.Printf("  ✓ FEC encoding successful: created %d parity packets\n", len(parityPackets))
			passedTests++
		}
	} else {
	...
	processor := packet.NewProcessor(100) // 100 packet buffer
	if processor != nil {
		fmt.Println("  ✓ Packet processor created")

		// Create test packets
		testPackets := []*protocol.Packet{
			{SequenceID: 3, Data: []byte("Packet 3")},
			{SequenceID: 1, Data: []byte("Packet 1")},
			{SequenceID: 2, Data: []byte("Packet 2")},
		}

		// Process out-of-order packets
		for _, pkt := range testPackets {
			processor.ProcessPacket(pkt)
		}
	...
	encoded, err := packet.EncodePacket(originalPacket)
	if err != nil {
		...
		// Decode it back
		decoded, err := packet.DecodePacket(encoded)
		if err != nil {
